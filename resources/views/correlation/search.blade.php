...
<nav class="navbar navbar-default navbar-fixed-top" style="padding: 20px;">
    <div class="container-fluid">
        ...
        <ul class="nav navbar-nav">
            <li>
                <form method="GET" action="{{url('/correlation/search')}}" id="form-konten">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        @for($i = 1; $i <= $totalPage; $i++)
                            <li class="page-item"><a class="page-link" href="{{url('/correlation/search?keyword=')}}{{$keyword}}&p={{$i}}">{{$i}}</a></li>
                        @endfor
                    </ul>
                </nav>
